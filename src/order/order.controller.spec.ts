...
describe("OrderController", () => {
    ...
    beforeEach(async () => {
        ...
        jest.spyOn(orderServiceMock, "findOrder").mockResolvedValueOnce(
            testOrder as Order,
        );
    });
    ...
    describe("udpdateAddress", () => {
        ...
        it("should return updatedOrder with new address", async () => {
            jest.spyOn(orderServiceMock, "updateAddress").mockResolvedValueOnce(
                { ...testOrder, orderAddress: "new_address" } as Order,
            );

            const order = await orderController.updateAddress(
                updateAddressDto,
                0,
            );
        ...
        it("should throw error if user doesn't have ownership", async () => {
            expect(
                orderController.updateAddress(updateAddressDto, 1),
    ...
    describe("updateStatus", () => {
        ...
        it("should throw error if user doesn't have ownership", async () => {
            ...
            expect(
                orderController.updateStatus(updateStatusDto, payload),
            ).rejects.toThrow(ForbiddenException);
        });
