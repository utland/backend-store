...
describe("OrderController", () => {
    ...
    describe("udpdateAddress", () => {
        ...
        it("should throw error if user doesn't have ownership", async () => {
            expect(orderController.updateAddress(updateAddressDto, 1))
                .rejects
    ...
    describe("updateStatus", () => {
        ...
        it("should throw error if user doesn't have ownership", async () => {
            ...
            expect(orderController.updateStatus(updateStatusDto, payload))
                .rejects
                .toThrow(ForbiddenException);
        })
        ...
        it("should update order if user have access", async () => {
            jest.spyOn(orderServiceMock, "updateStatus").mockResolvedValueOnce(
                { ...testOrder, status: OrderStatus.IN_PROCESS} as Order
            );
