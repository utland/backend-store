...
describe("UserService", () => {
    ...
    describe("findBy", () => {
        it("should throw exception if user is not found (by login)", async () => {
            ...
            await expect(userService.findByLogin("login")).rejects.toThrow(NotFoundException);
        });
    ...
    describe("changeRole", () => {
        it("should throw exception if user's role is the same", async () => {
            ...
            await expect(userService.changeRole(0, Role.USER)).rejects.toThrow(BadRequestException);
        });

        it("should throw exception if setting role is ADMIN", async () => {
            jest.spyOn(userRepo, "findOne").mockResolvedValueOnce(testUser as User);

            await expect(userService.changeRole(0, Role.ADMIN)).rejects.toThrow(BadRequestException);
        });
        ...
        it("should throw exception if changing role of ADMIN", async () => {
            ...
            await expect(userService.changeRole(0, Role.MODERATOR)).rejects.toThrow(BadRequestException);
        });
    ...
    describe("changePass", () => {
        ...
        it("should throw exception if password is incorrect", async () => {
            ...
            await expect(userService.changePass(changePassDto)).rejects.toThrow(NotAcceptableException);
        });
